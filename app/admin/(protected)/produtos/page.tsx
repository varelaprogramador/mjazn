...
const categoryColors: Record<string, 'fire' | 'white' | 'outline' | 'dark'> = {
  camisetas: 'fire',
  polos: 'white',
  kits: 'fire',
  acessorios: 'outline',
...
export default async function AdminProdutos() {
  const [products, drops] = await Promise.all([getAllProducts(), getAllDrops()])
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-gray-border bg-dark">
                <th className="text-left px-4 py-3 text-[10px] font-display tracking-widest text-gray-muted">
                  Produto
                </th>
                <th className="text-left px-4 py-3 text-[10px] font-display tracking-widest text-gray-muted hidden sm:table-cell">
                  Categoria
                </th>
                <th className="text-left px-4 py-3 text-[10px] font-display tracking-widest text-gray-muted">
                  Preço
                </th>
                <th className="text-left px-4 py-3 text-[10px] font-display tracking-widest text-gray-muted hidden md:table-cell">
                  Estoque
                </th>
                <th className="text-left px-4 py-3 text-[10px] font-display tracking-widest text-gray-muted hidden lg:table-cell">
                  Status
                </th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell">
                      <Badge
                        ...
                        variant={categoryColors[product.category] ?? 'outline'}
                      />
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-off-white">{formatPrice(product.price)}</p>
                      {product.originalPrice && (
                        <p className="text-gray-dim text-[10px] line-through">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <DeleteButton
                          ...
                          confirmMessage="Excluir produto?"
                        />
                      </div>
                    </td>
                  </tr>
